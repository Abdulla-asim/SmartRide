...
public:
    ...
    void changeLights()
    {
        if (type == 4)
        {
            ...
            lightRecord4 = (lightRecord4++) % type;
            signals[lightRecord4] = true;
        }
        if (type == 3)
        {
            signals[lightRecord3] = false;
            lightRecord3 = (lightRecord3++) % type;
...
TextBox createTextBox(int x, int y, int width, int height, const char* placeholder) {
    TextBox box;
    box.bounds = { (float)x, (float)y, (float)width, (float)height };
    box.active = false;
    strcpy(box.text, placeholder);
    return box;
}

void drawTextBox(TextBox& box) {
    if (box.active) {
        // Draw text box with active state
        DrawRectangleRec(box.bounds, LIGHTGRAY);
        DrawText(box.text, box.bounds.x + 5, box.bounds.y + 5, 20, BLACK);
    } else {
        // Draw text box with inactive state (placeholder)
        DrawRectangleRec(box.bounds, DARKGRAY);
        DrawText(box.text, box.bounds.x + 5, box.bounds.y + 5, 20, GRAY);
    }
}

bool checkButtonPressed(int x, int y, int width, int height) {
    Vector2 mousePos = GetMousePosition();
    return CheckCollisionPointRec(mousePos, { (float)x, (float)y, (float)width, (float)height });
}


void registerUser(vector<User>& users, TextBox userNameBox, TextBox userEmailBox, TextBox userPhoneBox, TextBox userLocationBox, TextBox userAgeBox)
{
    ...
    string location = userLocationBox.text;
    int userAge = stoi(userAgeBox.text);

    // Create a new User object and add it to the users vector
    User newUser(userAge, name, email, true, phone, location);
...
int main()
{
    std::vector<User> users;  // List to store users
    TextBox userNameBox, userEmailBox, userPhoneBox, userLocationBox, userAgeBox;
    int userAge = 18;  // Default age
    bool userRegistered = false;


    // Initialize Raylib
    InitWindow(800, 600, "User Registration for Ride Service");

    // Initialize text boxes and other UI components
    userNameBox = createTextBox(100, 100, 250, 30, "Name");
    userEmailBox = createTextBox(100, 150, 250, 30, "Email");
    userPhoneBox = createTextBox(100, 200, 250, 30, "Phone Number");
    userAgeBox = createTextBox(100, 250, 250, 30, "Age");
    userLocationBox = createTextBox(100, 250, 250, 30, "Location");
    ...
    while (!WindowShouldClose()) {
        ...
        // Draw UI elements
        DrawText("User Registration", 300, 50, 20, DARKGRAY);

        // Draw the input fields
        drawTextBox(userNameBox);
        drawTextBox(userEmailBox);
        drawTextBox(userPhoneBox);
        drawTextBox(userLocationBox);

        // Draw the Submit button
        if (userRegistered) {
            DrawText("User Registered! You can now request a ride.", 200, 350, 20, GREEN);
        } else {
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && checkButtonPressed(350, 300, 100, 40)) {
                registerUser(users, userNameBox, userEmailBox, userPhoneBox, userLocationBox, userAgeBox);  // Register the user
            }
            DrawRectangle(350, 300, 100, 40, DARKBLUE);
            DrawText("Register", 375, 310, 20, RAYWHITE);
        }

        EndDrawing();
    }

    // Close window and open OpenGL context
    CloseWindow();
